...
// version 23.3.1: 10.12.17 das for Shuchir
function drawProvidersChart($jsonObj, controlID, subcontrols, fldLabName, fldValue, providerName, strtYr, endYr, index, Min, Max, Units, seriescolors, chartTypeCode) {
    ...
    // STEPTOE EDIT BEGIN 12/19/16 Remove AgPumped if not regional
    if (fldnames.indexOf('MGWPUMP') > -1 && providerName.reg == undefined) {
        fldnames = fldnames.replace(",AGPUMPED", "");
    }
    // STEPTOE EDIT END 12/19/16 Remove AgPumped if not regional
            ...
            if (this.PVC) {
                ...
                if ((pcode != RegionCode) || (providerName.doreg)) {
                    ...
                    if (pname != undefined) {
                        ...
                        prvdrs.push(pname);
                        // ok get each year value and put into chart data array
    ...
    //console.log(divID, "currentChartObject undefined");
    var $myChart = {
    ...
    // This code block checks to see if the highchart is defined. If it is not, it creates it.
    // If it is defined, it updates the data using the options previously defined
    // 12.14.17 DAS and mostly Michael StepToe
    // ----------------------------------------------------
    var currentChartObject = $('#' + divID).highcharts();
    if (currentChartObject == undefined) {
        var chart = new Highcharts.Chart($myChart);
    }
    else {
        //console.log("currentChartObject defined");
        var options = currentChartObject.options;

        for (var option in $myChart) {
            options[option] = $myChart[option];
        }
        var chart = new Highcharts.Chart(options);
    }
    // ===================================================
   // console.log("Providers Chart");
