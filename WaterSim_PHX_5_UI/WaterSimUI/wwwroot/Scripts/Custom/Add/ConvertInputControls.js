...
var divPopup = '<div class="popup-container"><div id="#DID#PopupButton" class="ui-slider-popup-button"></div></div>';
...
var inputNumber = '<input id="#DID#number" name="#DID#number" class="input-number">';
...
function buildRadioInputControl(id, fieldName, controlOptions, subFields) {
    var html = "";
    ...
    //If input control has dependents display popup button
    if (subFields) {
        html += divPopup;
    }
...
function inputControl2Radio(inputControl, options) {
    ...
    //If input control has dependents display popup button
    if ($(inputControl).attr("data-Subs") != "") {
        subFields = true;
    }

    //Get the html to add the radio input control to the page and insert it after the slider
    var html = buildRadioInputControl(id, fieldName, inputControlRadioValues[fieldName], subFields);
    ...
    //On input text change update which label is highlighted based on inputRange
    //If the value is out of the min/max range adjust accordingly
    //Otherwise update based on which inputRange the value fits into
...
function inputControls2Radios() {
    ...
    //STEPTOE BEGIN ADD 02/08/17 
    //getting dependent controls
    $('#panelDependents .InputControl').each(function () {
        //Get the default value for the control from the slider
        inputControlRadioValues[$(this).attr("data-key")].checked = $(this).find('.InputSliderControl').slider("option", "value");
        //Convert from input control to a radio control
        inputControl2Radio(this, { height: 39 });
    });
    setFontSize('panelDependents');
    //STEPTOE END ADD 02/08/17
