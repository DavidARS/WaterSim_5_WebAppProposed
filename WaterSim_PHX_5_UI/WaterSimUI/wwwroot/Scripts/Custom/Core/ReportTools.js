    ...
    try{
        if (targetWindow.document.write) {
            if (aChartArray.length) {
                ...
                if (ChartN > 0 && groupTitle.indexOf("Previous Results Charts") == -1) {
        ...
        if (WinReady) {
            ...
            $(IndicatorControlsArray).each(function () {
                ...
                if (this.Description != undefined) {
                    si_description = this.Description;
                }

                IndicatorValues[cnt] = this.value;
                ...
                "<td style='height:110px; border:1px solid black'><p style='font-size:90%'><b>" + sectlab + "</b><br>&nbsp;&nbsp;&nbsp;Value = " + this.value.toString() + "</p></td><td><p>"+si_description+"<p></td></tr>";
            ...
            if (TestIndicators) {
                ReportWindow.initializeIndicators();
...
$("#ODUInputUserControl_ControlContainer").hide();
...
function onCreateOutDoorControl(e) {
    if (!outdoorControlHidden) {
        ...
        // increment count
        $("#ODUInputUserControl_ControlContainer").hide();
        $("#ODUInputUserControl_divslider").attr("data-def", 100);
        setSlider();
        ...
        $("#ODUInputUserControl_ControlContainer").show();
        $("#OutDoorWaterControl").css('color', 'red');
        outdoorControlHidden = false;
    }
}
//===================================
var AgefficiencyControlHidden = true;
$("#AGEFFInputUserControl_ControlContainer").hide();
$("#AgefficiencyControl").click(onCreateAgEffControl);

function onCreateAgEffControl(e) {
    if (!AgefficiencyControlHidden) {
        $("#AgefficiencyControl").css('color', 'black');
        // increment count
        $("#AGEFFInputUserControl_ControlContainer").hide();
        $("#AGEFFInputUserControl_divslider").attr("data-def", 100);
        setSlider();
        //Optionally you can force a model rerun here so that the system reflects the default for that control
        runModel();
        AgefficiencyControlHidden = true;
    }
    else {
        $("#AGEFFInputUserControl_ControlContainer").show();
        $("#AgefficiencyControl ").css('color', 'red');
        AgefficiencyControlHidden = false;
    }
}